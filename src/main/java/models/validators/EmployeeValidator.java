...
public class EmployeeValidator {
	...
	public static List<String> validate(EmployeeService service, EmployeeView ev, Boolean codeDuplicateCheckFlag, Boolean passwordCheckFlag){
		...
		if(!codeError.equals("")) errors.add(codeError);

		//氏名のチェック
		String nameError = validateName(ev.getName());
		if(!nameError.equals("")) errors.add(nameError);

		//パスワードのチェック
		String passError = validatePassword(ev.getPassword(), passwordCheckFlag);
		if(!passError.equals("")) errors.add(passError);
	...
	private static String validateCode(EmployeeService service, String code, Boolean codeDuplicateCheckFlag) {
		//入力値がなければエラーメッセージを返却
		if(code == null || code.equals("")) return MessageConst.E_NOEMP_CODE.getMessage();
	...
	/*
	 * 氏名に入力値があるかチェックし、入力値がなければエラーメッセージを返却
	 * @param name 氏名
	 * @return エラーメッセージ
	 */
	private static String validateName(String name) {
		if(name == null || name.equals("")) return MessageConst.E_NONAME.getMessage();
	...
	private static String validatePassword(String password, Boolean passwordCheckFlag) {
		...
		if(passwordCheckFlag && (password == null || password.equals(""))) return MessageConst.E_NOPASSWORD.getMessage();
