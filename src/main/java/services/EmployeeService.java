...
public class EmployeeService extends ServiceBase {
	...
	public EmployeeView findOne(String code, String plainPass, String pepper) {
		...
		try {
			...
			//社員番号とハッシュ和美パスワードを条件に未削除の従業員を1件取得する
	...
	/*
	 * 画面から入力された従業員の登録内容を基にデータを１件取得し、従業員テーブルに登録する
	...
	/*
	 * 画面から入力された従業員の更新内容を基にデータを１件作成し、従業員テーブルを更新する
	 * @param ev 画面から入力された従業員の登録内容
	 * @param pepper pepper文字列
	...
	public List<String> update(EmployeeView ev, String pepper){
		...
		if(errors.size() == 0) update(savedEmp);

		//エラーを返却（エラーがなければ0件のリスト）
		return errors;
	}
	...
	public Boolean validateLogin(String code, String plainPass, String pepper) {
		...
		if(code != null && !code.equals("") && plainPass != null && !plainPass.equals("")) {
			...
			//データが取得できた場合、認証成功
			if(ev != null && ev.getId() != null) isValidEmployee = true;
		}
